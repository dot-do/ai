---
$id: https://integrations.do/scrapfly
$type: Integration
name: Scrapfly
service: scrapfly
description: Scrapfly is a web scraping API that enables developers to extract data from websites efficiently, offering features like JavaScript rendering, anti-bot protection bypass, and proxy rotation.
category: productivity
sdkBased: false
baseUrl: https://api.scrapfly.com

auth:
  type: api-key
  location: header
  headerName: Authorization
  scheme: Bearer

resources:
  - name: Action
    plural: Actions
    description: Execute Scrapfly actions
    operations:
      - name: execute
        method: POST
        path: /
        params:
          - name: action
            type: string
            required: true
            description: Action name to execute
          - name: parameters
            type: object
            required: false
            description: Action parameters
        returns: object

webhooks:
  enabled: false
  events: []

errors:
  mapping:
    - code: 401
      type: authentication
      statusCode: 401
      message: Unauthorized
      retryable: false
    - code: 429
      type: rate_limit
      statusCode: 429
      message: Rate limit exceeded
      retryable: true
  retryable: [429, 500]
  nonRetryable: [400, 401, 403, 404]

tests:
  enabled: true
  cleanup: false
  scenarios:
    - name: Basic Action Execution
      description: Test basic action execution
      steps:
        - action: custom
          resource: Action

docs:
  homepage: https://scrapfly.io/
  apiDocs: https://scrapfly.io/
---

# Scrapfly Integration

Scrapfly is a web scraping API that enables developers to extract data from websites efficiently, offering features like JavaScript rendering, anti-bot protection bypass, and proxy rotation.

## Quick Start

```typescript
import { ScrapflyClient } from '@dotdo/sdk.do/integrations/scrapfly'

const scrapfly = new ScrapflyClient({
  apiKey: process.env.SCRAPFLY_API_KEY,
})

const result = await scrapfly.action.execute({
  action: 'example_action',
  parameters: {},
})
```

## Actions

Scrapfly provides 2 actions through the Composio API.

## License

MIT
