---
$id: https://integrations.do/documint
$type: Integration
name: Documint
service: documint
...
resources:
  - name: Action
    plural: Actions
    description: Execute Documint actions
    operations:
      - name: execute
        method: POST
        path: /
...
```typescript
import { DocumintClient } from '@dotdo/sdk.do/integrations/documint'

const documint = new DocumintClient({
  apiKey: process.env.DOCUMINT_API_KEY,
})

const result = await documint.action.execute({
...
Documint provides 5 actions through the Composio API.
