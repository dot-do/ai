---
$id: https://integrations.do/pagerduty
$type: Integration
name: Pagerduty
service: pagerduty
description: Integrate PagerDuty to manage incidents, schedules, and alerts directly from your application.
category: developer-tools
sdkBased: false
baseUrl: https://api.pagerduty.com

auth:
  type: oauth2
  authorizationUrl: https://app.pagerduty.com/oauth/authorize
  tokenUrl: https://app.pagerduty.com/oauth/token
  scopes:
    - read
    - write

resources:
  - name: Action
    plural: Actions
    description: Execute Pagerduty actions
    operations:
      - name: execute
        method: POST
        path: /
        params:
          - name: action
            type: string
            required: true
            description: Action name to execute
          - name: parameters
            type: object
            required: false
            description: Action parameters
        returns: object

webhooks:
  enabled: false
  events: []

errors:
  mapping:
    - code: 401
      type: authentication
      statusCode: 401
      message: Unauthorized
      retryable: false
    - code: 429
      type: rate_limit
      statusCode: 429
      message: Rate limit exceeded
      retryable: true
  retryable: [429, 500]
  nonRetryable: [400, 401, 403, 404]

tests:
  enabled: true
  cleanup: false
  scenarios:
    - name: Basic Action Execution
      description: Test basic action execution
      steps:
        - action: custom
          resource: Action

docs:
  homepage: https://www.pagerduty.com
  apiDocs: https://www.pagerduty.com
---

# Pagerduty Integration

Integrate PagerDuty to manage incidents, schedules, and alerts directly from your application.

## Quick Start

```typescript
import { PagerdutyClient } from '@dotdo/sdk.do/integrations/pagerduty'

const pagerduty = new PagerdutyClient({
  apiKey: process.env.PAGERDUTY_API_KEY,
})

const result = await pagerduty.action.execute({
  action: 'example_action',
  parameters: {},
})
```

## Actions

Pagerduty provides 354 actions through the Composio API.

## License

MIT
