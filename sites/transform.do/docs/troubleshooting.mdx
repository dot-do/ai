---
$id: https://transform.do/docs/troubleshooting
$type: TechArticle
title: transform.do Troubleshooting
description: Common issues and solutions for transform.do
keywords: [transform, pipeline, data, processing, etl, mapping, filter, reduce, map, troubleshooting, issues, debugging, solutions]
---

# transform.do Troubleshooting

Common issues and their solutions when working with transform.do.

## Installation Issues

### Package Not Found

**Problem**: Cannot find package transform.do

```
Error: Cannot find package 'transform.do'
```
...
# Or specify registry
pnpm add transform.do --registry=https://registry.npmjs.org
```
...
```
Error: TRANSFORM_API_KEY is required
```

**Solution**: Set environment variable:

```bash
export TRANSFORM_API_KEY=your-key-here
```
...
await transform.processStream(stream)
```
...
```typescript
const result = await transform<MyType>(data)
```
...
```bash
# Check versions
pnpm list transform.do sdk.do

# Update if needed
pnpm update transform.do sdk.do
```
...
const instance = transform.create(config)
