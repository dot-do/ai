---
$id: https://ari.do/docs/troubleshooting
$type: TechArticle
title: Recruiting AI Agent Troubleshooting
description: Common issues and solutions for Recruiting AI Agent
keywords: [troubleshooting, ari.do, debugging, issues, solutions]
author:
  $type: Organization
  name: .do Platform
---
...
Common issues and solutions when working with Recruiting AI Agent.
...
const results = await Promise.all([recruitingAgent.execute(task1), recruitingAgent.execute(task2), recruitingAgent.execute(task3)])
```
...
for (const [name, tool] of Object.entries(recruitingAgent.tools)) {
