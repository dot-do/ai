---
$id: https://ceo.do/docs/troubleshooting
$type: TechArticle
title: CEO Strategic Agent Troubleshooting
description: Common issues and solutions for CEO Strategic Agent
keywords: [troubleshooting, ceo.do, debugging, issues, solutions]
author:
  $type: Organization
  name: .do Platform
---
...
Common issues and solutions when working with CEO Strategic Agent.
...
const results = await Promise.all([ceoAgent.execute(task1), ceoAgent.execute(task2), ceoAgent.execute(task3)])
```
...
for (const [name, tool] of Object.entries(ceoAgent.tools)) {
