---
$id: https://validate.do/docs/troubleshooting
$type: TechArticle
title: validate.do Troubleshooting
description: Common issues and solutions for validate.do
keywords: [validate, validation, schema, rules, types, zod, yup, constraints, troubleshooting, issues, debugging, solutions]
---

# validate.do Troubleshooting

Common issues and their solutions when working with validate.do.

## Installation Issues

### Package Not Found

**Problem**: Cannot find package validate.do

```
Error: Cannot find package 'validate.do'
```
...
# Or specify registry
pnpm add validate.do --registry=https://registry.npmjs.org
```
...
```
Error: VALIDATE_API_KEY is required
```

**Solution**: Set environment variable:

```bash
export VALIDATE_API_KEY=your-key-here
```
...
await validate.processStream(stream)
```
...
try {
  ...
  const result = await validate(validated)
} catch (error) {
...
```typescript
const result = await validate<MyType>(data)
```
...
```bash
# Check versions
pnpm list validate.do sdk.do

# Update if needed
pnpm update validate.do sdk.do
```
...
const instance = validate.create(config)
