/**
 * Authentication Commands for agents.do CLI
 *
 * Manages CLI authentication for agent orchestration.
...
export function createAuthCommands(): Command {
  const authCommand = new Command('auth').description('Manage agents.do authentication')
    ...
    .action(async () => {
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.cyan('  • OAuth:    agents auth login'))
  ...
  authCommand
    ...
    .action(async (token: string) => {
      ...
      try {
        ...
        console.log(chalk.gray('\nYou can now use agents CLI commands with this API key.\n'))
      } catch (error) {
    ...
    .action(async () => {
      ...
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.yellow('\nRun "agents auth login" to authenticate.\n'))
          process.exit(1)
        }
        ...
        if (!validation.active) {
          ...
          console.log(chalk.yellow('\nRun "agents auth login" to re-authenticate.\n'))
          process.exit(1)
        }
