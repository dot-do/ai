/**
 * Authentication Commands for SDK.do CLI
 *
 * Manages CLI authentication for SDK.do connections.
...
export function createAuthCommands(): Command {
  const authCommand = new Command('auth').description('Manage SDK.do authentication')
    ...
    .action(async (options) => {
      try {
        if (!isKeychainAvailable()) {
          ...
          console.error(chalk.yellow('Alternative: Use API key authentication via SDK_TOKEN'))
          process.exit(1)
        }
    ...
    .action(async () => {
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.cyan('  • OAuth:    sdk auth login'))
          console.log(chalk.cyan('  • API Key:  export SDK_TOKEN=sk_xxx'))
          console.log(chalk.cyan('  • Admin:    export SDK_ADMIN_TOKEN=sk_admin_xxx\n'))
          process.exit(1)
        }
  ...
  authCommand
    ...
    .action(async (token: string) => {
      ...
      try {
        ...
        console.log(chalk.gray('\nYou can now use SDK.do CLI commands with this API key.\n'))
      } catch (error) {
  ...
  /**
   * Set Admin Token (SDK_ADMIN_TOKEN)
   */
  authCommand
    ...
    .action(async () => {
      ...
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.yellow('\nRun "sdk auth login" to authenticate.\n'))
          process.exit(1)
        }
        ...
        if (!validation.active) {
          ...
          console.log(chalk.yellow('\nRun "sdk auth login" to re-authenticate.\n'))
          process.exit(1)
        }
...
function formatAuthType(type: 'admin' | 'oauth' | 'apikey'): string {
  switch (type) {
    case 'admin':
      return chalk.red('Admin Token (SDK_ADMIN_TOKEN)')
    case 'oauth':
      return chalk.green('OAuth (WorkOS)')
    case 'apikey':
      return chalk.blue('API Key (SDK_TOKEN)')
    default:
      return type
  }
}
