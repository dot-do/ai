 ...
 *
 * Manages CLI authentication for MCP server connections.
 * Uses auth.do for shared authentication with OAuth and API keys.
 */
...
export function createAuthCommands(): Command {
  const authCommand = new Command('auth').description('Manage MCP authentication')
    ...
    .action(async (options) => {
      try {
        if (!isKeychainAvailable()) {
          ...
          console.error(chalk.yellow('Alternative: Use API key authentication via MCP_TOKEN'))
          process.exit(1)
        }
    ...
    .action(async () => {
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.cyan('  • OAuth:    mcp auth login'))
          console.log(chalk.cyan('  • API Key:  export MCP_TOKEN=sk_xxx'))
          console.log(chalk.cyan('  • Admin:    export MCP_ADMIN_TOKEN=sk_admin_xxx\n'))
          process.exit(1)
        }
  ...
  authCommand
    ...
    .action(async (token: string) => {
      ...
      try {
        ...
        console.log(chalk.gray('\nYou can now use MCP CLI commands with this API key.\n'))
      } catch (error) {
    ...
    .action(async () => {
      ...
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.yellow('\nRun "mcp auth login" to authenticate.\n'))
          process.exit(1)
        }
        ...
        if (!validation.active) {
          ...
          console.log(chalk.yellow('\nRun "mcp auth login" to re-authenticate.\n'))
          process.exit(1)
        }
...
function formatAuthType(type: 'admin' | 'oauth' | 'apikey'): string {
  switch (type) {
    case 'admin':
      return chalk.red('Admin Token (MCP_ADMIN_TOKEN)')
    case 'oauth':
      return chalk.green('OAuth (WorkOS)')
    case 'apikey':
      return chalk.blue('API Key (MCP_TOKEN)')
    default:
      return type
  }
}
