 ...
 *
 * Manages CLI authentication via OAuth and API keys.
 * Supports: OAuth (WorkOS), DO_TOKEN, DO_ADMIN_TOKEN
 */
...
export function createAuthCommands(): Command {
    ...
    .action(async () => {
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.cyan('  • OAuth:    do auth login'))
  ...
  authCommand
    ...
    .action(async (token: string) => {
      ...
      try {
        ...
        console.log(chalk.gray('\nYou can now use CLI commands with this API key.\n'))
      } catch (error) {
  ...
  /**
   * Set Admin Token (DO_ADMIN_TOKEN)
   */
  authCommand
    ...
    .action(async () => {
      ...
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.yellow('\nRun "do auth login" to authenticate.\n'))
          process.exit(1)
        }
        ...
        if (!validation.active) {
          ...
          console.log(chalk.yellow('\nRun "do auth login" to re-authenticate.\n'))
          process.exit(1)
        }
