/**
 * Authentication Commands for mdxai CLI
 *
 * Manages CLI authentication for AI-powered MDX operations.
...
export function createAuthCommands(): Command {
  const authCommand = new Command('auth').description('Manage mdxai authentication')
    ...
    .action(async () => {
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.cyan('  • OAuth:    mdxai auth login'))
  ...
  authCommand
    ...
    .action(async (token: string) => {
      ...
      try {
        ...
        console.log(chalk.gray('\nYou can now use mdxai CLI commands with this API key.\n'))
      } catch (error) {
    ...
    .action(async () => {
      ...
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.yellow('\nRun "mdxai auth login" to authenticate.\n'))
          process.exit(1)
        }
        ...
        if (!validation.active) {
          ...
          console.log(chalk.yellow('\nRun "mdxai auth login" to re-authenticate.\n'))
          process.exit(1)
        }
