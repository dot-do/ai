 ...
 * Shopify Webhooks
 *
 * Auto-generated webhook handling for Shopify Integration.
 ...
 */
export interface ProductsCreateEvent {
...
export interface OrdersCreateEvent {
  type: 'orders/create'
  data: any
  timestamp: string
  id: string
...
export type WebhookEvent = ProductsCreateEvent | OrdersCreateEvent
...
export class ShopifyWebhookHandler {
  ...
  verify(options: WebhookVerificationOptions): boolean {
    ...
    return this.secureCompare(options['X-Shopify-Hmac-Sha256'], expectedSignature)
  }
  ...
  async handleRequest(request: Request): Promise<WebhookEvent> {
    ...
    if (!this.verify({ secret: this.secret, 'X-Shopify-Hmac-Sha256': signature, payload })) {
...
export class WebhookEventRouter {
  ...
  /**
   * Register handler for products/create events
   * @param handler - Event handler function
   */
  onProductsCreate(handler: (event: ProductsCreateEvent) => Promise<void>): void {
    this.on(WEBHOOK_EVENTS.PRODUCTS_CREATE, handler as any)
  }

  /**
   * Register handler for orders/create events
   * @param handler - Event handler function
   */
  onOrdersCreate(handler: (event: OrdersCreateEvent) => Promise<void>): void {
    this.on(WEBHOOK_EVENTS.ORDERS_CREATE, handler as any)
  }
