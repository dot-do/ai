/**
 * Authentication Commands for mdxdb CLI
 *
 * Manages CLI authentication for MDX database operations.
...
export function createAuthCommands(): Command {
  const authCommand = new Command('auth').description('Manage mdxdb authentication')
    ...
    .action(async () => {
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.cyan('  • OAuth:    mdxdb auth login'))
  ...
  authCommand
    ...
    .action(async (token: string) => {
      ...
      try {
        ...
        console.log(chalk.gray('\nYou can now use mdxdb CLI commands with this API key.\n'))
      } catch (error) {
    ...
    .action(async () => {
      ...
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.yellow('\nRun "mdxdb auth login" to authenticate.\n'))
          process.exit(1)
        }
        ...
        if (!validation.active) {
          ...
          console.log(chalk.yellow('\nRun "mdxdb auth login" to re-authenticate.\n'))
          process.exit(1)
        }
