/**
 * Authentication Commands for mdxe CLI
 *
 * Manages CLI authentication for MDX execution operations.
...
export function createAuthCommands(): Command {
  const authCommand = new Command('auth').description('Manage mdxe authentication')
    ...
    .action(async () => {
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.cyan('  • OAuth:    mdxe auth login'))
  ...
  authCommand
    ...
    .action(async (token: string) => {
      ...
      try {
        ...
        console.log(chalk.gray('\nYou can now use mdxe CLI commands with this API key.\n'))
      } catch (error) {
    ...
    .action(async () => {
      ...
      try {
        ...
        if (!authInfo) {
          ...
          console.log(chalk.yellow('\nRun "mdxe auth login" to authenticate.\n'))
          process.exit(1)
        }
        ...
        if (!validation.active) {
          ...
          console.log(chalk.yellow('\nRun "mdxe auth login" to re-authenticate.\n'))
          process.exit(1)
        }
