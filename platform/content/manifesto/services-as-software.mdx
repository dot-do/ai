...
const contentService = Service({
  ...
  objective: 'Create marketing copy quickly',
  keyResults: ['Reduce content creation time by 50%', 'Maintain content quality score ≥ 8'],
  pricing: {
    model: 'cost-based',
    costBase: 10,
    fixedCosts: 5,
...
await contentService.register()
const price = contentService.calculatePrice({ quantity: 5 })
```
