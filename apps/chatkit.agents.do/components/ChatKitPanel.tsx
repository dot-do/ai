...
import { STARTER_PROMPTS, PLACEHOLDER_INPUT, GREETING, CREATE_SESSION_ENDPOINT, WORKFLOW_ID } from '@/lib/config'
...
export function ChatKitPanel({ theme, onWidgetAction, onResponseEnd, onThemeRequest }: ChatKitPanelProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('chatkit-script-error', handleError as (event: Event) => void)
    ...
    return () => {
      ...
      window.removeEventListener('chatkit-script-error', handleError as (event: Event) => void)
      if (timeoutId) {
