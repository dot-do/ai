...
export function replaceWikilinks(text: string): string {
  return text.replace(/\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g, (match, link, alias) => {
    ...
    return `<a href="${url}" class="wikilink">${displayText}</a>`
  })
}
...
export function extractBacklinks(content: string, targetSlug: string): Backlink[] {
  // This would be implemented with a full-text search across all pages
  // For now, return empty array (to be implemented with R2 SQL or Vectorize)
  return []
}
