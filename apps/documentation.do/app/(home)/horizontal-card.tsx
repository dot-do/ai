...
export function HorizontalSection({ title, description, docsLink, otherLink, withMeteors, titleClass, children }: HorizontalSectionProps) {
  return (
    <section
      className={cn(
    ...
    >
      ...
      <div className="flex h-full flex-col gap-y-2 xl:gap-y-4">
        <h2 className={cn('text-2xl font-bold sm:text-3xl md:text-4xl lg:text-5xl xl:text-6xl', titleClass)}>{title}</h2>
        <p className="text-muted-foreground max-w-xl text-pretty text-sm font-medium sm:text-base md:text-lg xl:text-xl">{description}</p>
