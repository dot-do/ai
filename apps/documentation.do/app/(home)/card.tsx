...
export const Card = ({ icon, title, description, link, children }: CardProps) => {
  ...
  return (
    ...
    >
      ...
      <header className="flex items-center justify-between">
        ...
        {link && (
          <>
            {/* @ts-expect-error - React 18 types compatibility issue with Next.js Link */}
            <Link href={link} className="text-muted-foreground hover:text-foreground mr-1 text-3xl font-extralight transition-colors">
              {'↗'}
            </Link>
          </>
        )}
      ...
      <main className="flex flex-col">
        <p className="text-muted-foreground my-2 text-pretty text-sm sm:text-base lg:my-4 xl:text-lg">{description}</p>
        {children}
      </main>
    </article>
  )
