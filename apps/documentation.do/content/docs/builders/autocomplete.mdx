...
<Links>
  <Link label="Source" icon="github" link="https://github.com/sunarjs/sunar/blob/main/packages/sunar/src/builders/autocomplete.ts" />
  <Link label="Autocomplete Interaction" icon="discordjs" link="https://discord.js.org/docs/packages/discord.js/main/AutocompleteInteraction:Class" />
  <Link label="API Reference" icon="discord" link="https://discord.com/developers/docs/interactions/application-commands#autocomplete" />
</Links>
...
execute(autocomplete, (interaction, option) => {
  ...
  const results = data.filter((e) => e.name.toLowerCase().includes(option.value.toLowerCase()))
